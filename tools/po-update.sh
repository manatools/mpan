...
intltool-extract --type=gettext/ini extras/mpan.desktop.in
intltool-extract --type=gettext/xml conf/categories.conf.in
...
/usr/bin/xgettext \
	...
	--output="$POT_FILE" \
	mpan/mpan \
	extras/mpan.desktop.in.h \
	conf/categories.conf.in.h

/bin/sed --in-place --expression="s/charset=CHARSET/charset=UTF-8/" "$POT_FILE"

intltool-merge --desktop-style po extras/mpan.desktop.in extras/mpan.desktop

rm -f extras/mpan.desktop.in.h
rm -f conf/categories.conf.in.h
